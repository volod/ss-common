...
- `RUN NEEDED`: implementation is deterministic, but acceptance includes a declared heavier run
  (for example, a multi-architecture build or a resolution against the package index).
...
- `BLOCKED BY HUMAN`: an agent prepares support, but a human-provided artifact gates completion
  (hardware, a pushed repository, a published release).
...
Both lint targets run inside `make ci`. The implementation lives in `src/ss_kit/quality/`. When a check fails, fix the documents, not the checker.
