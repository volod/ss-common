...
from ss_kit.quality.plan_summary import main, summary_lines
...
def test_repository_plan_summary_reports_each_lane() -> None:
    lines = summary_lines(PROJECT_ROOT)

    assert lines[0].startswith("tasks: ")
    # While staged, the forward work lives in the staging repository's plan.
    assert "next agent: none" in lines
    assert "next human: none" in lines
